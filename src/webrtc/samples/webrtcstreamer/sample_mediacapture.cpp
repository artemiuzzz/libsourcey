...
SampleMediaCapture::SampleMediaCapture()
    : MediaCapture()
//    : _formatCtx(nullptr)
//    , _video(nullptr)
//    , _audio(nullptr)
...
void SampleMediaCapture::openStream(const std::string& filename, AVInputFormat* inputFormat, AVDictionary** formatParams)
{
    ...
    for (unsigned i = 0; i < _formatCtx->nb_streams; i++) {
        ...
        if (!_video && codec->codec_type == AVMEDIA_TYPE_VIDEO) {
            _video = new RawVideoDecoder(stream);
            _video->emitter.attach(packetSlot(this, &SampleMediaCapture::emit));
            _video->create();
            _video->open();
        }
    }
...
void SampleMediaCapture::emit(IPacket& packet)
{
