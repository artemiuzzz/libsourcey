...
RawVideoPacketSource::RawVideoPacketSource(int width, int height, int fps, uint32_t fourcc)
...
RawVideoPacketSource::RawVideoPacketSource(const cricket::VideoFormat& captureFormat)
...
RawVideoPacketSource::~RawVideoPacketSource()
{
    LDebug(": Destroying")
}


void RawVideoPacketSource::setPacketSource(PacketSignal* source)
{
    assert(!_source);
    // if (_source)
    //     _source->detach(packetSlot(this, &RawVideoPacketSource::onVideoCaptured));
    _source = source;
}


cricket::CaptureState RawVideoPacketSource::Start(const cricket::VideoFormat& format)
{
    LDebug("Start")
    ...
    if (_source)
        _source->attach(packetSlot(this, &RawVideoPacketSource::onVideoCaptured));
...
void RawVideoPacketSource::Stop()
{
    LDebug("Stop")
...
bool RawVideoPacketSource::IsRunning()
{
    return capture_state() == cricket::CS_RUNNING;
}


bool RawVideoPacketSource::GetPreferredFourccs(std::vector<uint32_t>* fourccs)
{
    if (!fourccs)
        return false;
...
bool RawVideoPacketSource::GetBestCaptureFormat(const cricket::VideoFormat& desired, cricket::VideoFormat* best_format)
{
    if (!best_format)
        return false;
...
bool RawVideoPacketSource::IsScreencast() const
{
    return false;
}
